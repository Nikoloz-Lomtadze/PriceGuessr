...
class GameManager:
    ...
    def save_game(self, session, won_override=None):
        ...
        cursor.execute( ## შევავასოთ ბაზა ახალი თამაშის მონაცემებით
            """
            INSERT INTO games
                (user_id, mode, rounds_played, rounds_won, won, points)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                session.user_id,
        ...
        for result in session.round_results:
            cursor.execute( ## შევავსოთ რაუნდების მონაცემები
                ...
                INSERT INTO game_rounds (
                    game_id, round_number, left_item_name, right_item_name,
                    chosen_item_name, correct_item_name, category_guessed,
                    won, points
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
